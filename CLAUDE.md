...
- `buildroot/packages/` — custom out-of-tree buildroot packages: `pd690xx` (PoE controller CLI/lib for MS220-P switches), `find_hdr` (buildroot-packaged version of the header-finder tool), `luton26ctl` (read-only `/dev/mem` diagnostic CLI for the Vitesse/Microchip Luton26 switch ASIC — `chipid`/`ports` subcommands, register offsets sourced from Microchip's MIT-licensed MESA API and validated against real MS220-24P hardware; a first step toward an eventual open driver for the switch ASIC, see Architecture notes), and `click` (Kohler's userspace Click modular router library — present but **not currently wired into any build**: no `buildroot/patches/*.patch` registers it in `package/Config.in`, and no board's `buildroot-config` sets `BR2_PACKAGE_CLICK=y`; don't confuse it with the actual switch dataplane, see Architecture notes)
- `buildroot/patches/` — patches applied to a **stock** buildroot source tree to register the out-of-tree `pd690xx`/`find_hdr`/`luton26ctl` packages above in `package/Config.in` (buildroot doesn't know about packages living outside its own tree, so these patches wire them in)
...
**There is an in-progress effort toward an open (non-proprietary) driver for the Luton26 switch ASIC**, tracked upstream in `halmartin/meraki-builder#4`. Microchip's MIT-licensed MESA switch API (`github.com/microchip-ung/mesa`, `base/luton26/`) has a complete register-level programming model for Luton26 (VSC7425/VSC7427) — validated against real MS220-24P hardware (`CHIP_ID` decodes to `PART_ID 0x7427` exactly). No equivalent exists for Jaguar1 (VSC7434, used in MS220-48/MS320-*/MS42) — MESA only covers the later, unrelated Jaguar2 chip. `buildroot/packages/luton26ctl` is the first concrete step: a read-only `/dev/mem` probe (`chipid`, `ports` subcommands), not yet a real kernel driver. Open item: the chip-internal `DEV_0`..`DEV_25` register block index does not map linearly to front-panel port numbers (confirmed by comparing `luton26ctl ports` / raw `PCS1G_LINK_STATUS` reads against the Click driver's own `/click/switch_port_table/dump_pports` — they disagree), most likely because the VSC7427 has only 12 built-in copper PHYs and the remaining ports run through external PHY chips over SERDES, whose chip-to-PHY link is always "up" regardless of real cable state.
